...
// CreditsUsageParts — shared primitives for the three Credits & Usage
// design variants (A stacked form, B guided stepper, C team dashboard).
// Extracted here once all three variants composed the same building
// blocks (rule of three). Layout/composition lives in each variant file;
// only the reusable atoms + their chrome live here.

export function cadenceShort(id) {
  return (CADENCES.find((c) => c.id === id) || CADENCES[1]).short;
}

// Contract-terms banner — the legal framing that opens every variant.
...
export function ComingSoonBadge() {
  return <span style={partStyles.p1Badge}>Coming soon</span>;
}
      ...
      {CADENCES.map((c) => {
        ...
        return (
          <button
            ...
            style={{
              ...partStyles.cadenceBtn,
              background: selected ? "var(--color-icon-tertiary-fg)" : "#FFFFFF",
              color: selected ? "#FFFFFF" : "var(--color-text-tertiary)",
...
// avoids the word "overage". Used by all three variants.
...
const partStyles = {
  ...
  cadenceWrap: { display: "inline-flex", gap: 6 },
  cadenceBtn: {
    appearance: "none",
    height: 32,
    padding: "0 14px",
