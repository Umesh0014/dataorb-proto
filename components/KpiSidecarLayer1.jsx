...
import { OutcomeBar, RagChip, SkeletonRows, sortAgents, statusLabelFor } from "./KpiSidecarParts";
...
export default function KpiSidecarLayer1({ kpi, onSelectAgent }) {
  ...
  const rateRag = kpi.campaignRate >= kpi.target ? "green" : "red";
  const delta = +(kpi.campaignRate - kpi.target).toFixed(1);
    ...
    <div style={s.wrap}>
      ...
      <div style={s.summary}>
        <div style={s.sumTop}>
          ...
          <div style={s.metaRow}>
            <span style={s.meta}><strong style={s.metaStrong}>{kpi.total.toLocaleString()}</strong> interactions · Target {kpi.target}{kpi.unit}</span>
