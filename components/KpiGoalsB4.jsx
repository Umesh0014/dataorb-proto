...
import { KPIS, CATEGORIES, DATE_RANGE, ON_TRACK_TOTAL } from "./mocks/kpiGoals";
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.split}>
        ...
        <aside style={s.rail}>
          <div style={s.overallTile}>
            <KpiRing pct={(ON_TRACK_TOTAL / KPIS.length) * 100} rag="amber" size={40} />
            <div>
              <span style={s.overallNum}>{ON_TRACK_TOTAL}/{KPIS.length}</span>
              <span style={s.overallSub}>on track</span>
            </div>
          </div>
          {CATEGORIES.map((c) => {
            const on = c.id === catId;
            return (
              <button key={c.id} type="button" onClick={() => pick(c.id)}
                style={{ ...s.railTile, ...(on ? s.railOn : null) }}>
                <div style={s.railTop}>
                  <KpiRing pct={c.score} rag={c.rag} size={38} label={c.score} />
                  <RagChip rag={c.rag} label={c.status} />
                </div>
                <span style={s.railName}>{c.name}</span>
                <span style={s.railMeta}>{c.onTrack} of {c.total} on track</span>
                <p style={s.railBlurb}>{c.blurb}</p>
              </button>
            );
          })}
        </aside>
...
const s = {
  ...
  overallTile: { display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", background: "var(--surface-alt)", borderRadius: 12 },
  overallNum: { display: "block", fontSize: 16, fontWeight: 800, color: "var(--color-text-deep)" },
  overallSub: { fontSize: 11, color: "var(--color-text-tertiary)" },
  railTile: { display: "flex", flexDirection: "column", gap: 6, padding: "14px 16px", border: "1px solid var(--color-divider-card)", borderRadius: 12, background: "#fff", cursor: "pointer", textAlign: "left", fontFamily: "var(--font-sans)", transition: "border-color .15s, box-shadow .15s" },
  railOn: { borderColor: "var(--do-brand-blue)", boxShadow: "0 0 0 1px var(--do-brand-blue)", background: "#FCFDFF" },
  railTop: { display: "flex", alignItems: "center", justifyContent: "space-between" },
  railName: { fontSize: 14, fontWeight: 800, color: "var(--color-text-deep)" },
  railMeta: { fontSize: 11.5, color: "var(--color-text-tertiary)" },
  railBlurb: { fontSize: 11.5, color: "var(--color-text-tertiary)", margin: "2px 0 0", lineHeight: 1.4 },
