    ...
    <div style={pbStyles.detail}>
      {designVer === "current" ? (
        ...
        <>
          ...
          {showV3I1 && <PlaybookV3I1 data={PLAYBOOK_V3_I1_MOCK} />}
          {opt === "O3" && !showV3I1 && <PlaybookOptionC {...shared} />}
        </>
      )}
      <VersionBar
        value={barValue}
...
// ---- v3 / i1 — compact accordion + bottom evidence strip + trace --------
// Reuses v1/i1's stage payload (Returns & cancellations workflow) but
// renders each stage as a collapsible accordion row; all rows are
// expanded by default. Closing carries the active highlight by default.
// A compact evidence strip pinned at the bottom shows overlapping
// agent avatars + the aggregate stats + a Trace link.
const PLAYBOOK_V3_I1_MOCK = {
  campaignName: "Returns & cancellations",
  keyTag: "Price pressure",
  whyText:
    "Up-front authentication and an early intent check kept the flow linear — resolved in one pass, no repetition.",
  stages: PLAYBOOK_V1_I1_MOCK.stages,
  agents: PLAYBOOK_AGENTS_GRID,
  summary: "Built from 4 agents · 27 source messages · all resolved",
  defaultActiveStageId: 6, // "Closing" — last stage, highlighted by default
};

function PlaybookV3I1({ data }) {
  // All stages start expanded; user can collapse individually.
  ...
  return (
    <div style={v3i1.body}>
      <div style={v3i1.heroRow}>
        <span style={v3i1.heroTile}>
          <BookOpen size={22} color="var(--color-icon-tertiary-fg)" />
        </span>
        <h3 style={v3i1.heroTitle}>{data.campaignName}</h3>
        <span style={v3i1.keyTag}>{data.keyTag}</span>
      </div>

      <blockquote style={v3i1.quote}>
        <p style={v3i1.quoteText}>{data.whyText}</p>
      </blockquote>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={v3i1.stageTitle}>{stage.name}</span>
        <span style={{ ...v3i1.refChip, background: ramp.bg, color: ramp.text }}>
          #{stage.refs[0]}–{stage.refs[1]}
        </span>
        <span
          style={{
      ...
      {expanded && (
        <div style={v3i1.stageBody}>
          {stage.bullets.map((b, i) => (
            <div key={i} style={v3i1.bullet}>
              <span aria-hidden="true" style={v3i1.bulletDot}>•</span>
              <span>{b}</span>
            </div>
          ))}
        </div>
...
function V3I1EvidenceStrip({ data }) {
  return (
    <div style={v3i1.evStrip}>
      <div style={v3i1.avStack}>
        {data.agents.map((a, i) => {
          const ramp = PB_AGENTS[a.id] || PB_AGENTS.SI;
          return (
            <span
              key={a.id}
              style={{
      ...
      <span style={v3i1.evText}>{data.summary}</span>
      <button type="button" style={v3i1.traceBtn}>
        Trace
      </button>
    </div>
  );
